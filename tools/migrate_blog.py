...
def main():
    ...
    def normalise(html, fname):
        ...
        # Likewise the Google tag, the consent defaults and conversions.js:
        # apply_analytics.py owns that block and adds it after this generator
        # runs, so a rendered page never has one and a page on disk always
        # does. Comparing without it is what keeps --check from reporting all
        # 24 blog pages as differing forever.
        html = re.sub(r'<!-- BEGIN google tag \(generated by tools/apply_analytics\.py\) -->'
                      r'.*?<!-- END google tag -->\n?', '', html, flags=re.S)
        # And the schema.org @graph, owned by apply_schema.py, which runs after
        # apply_analytics.py for the same reason and needs the same allowance.
        html = re.sub(r'<!-- BEGIN structured data \(generated by tools/apply_schema\.py\) -->'
                      r'.*?<!-- END structured data -->\n?', '', html, flags=re.S)
        # And the no-JavaScript navigation, owned by apply_crawl_nav.py. This
        # one sits *inside* the SiteFooter import, so removing it leaves the
        # indentation behind — collapse the emptied element back to the shape
        # this generator writes, or all 28 blog pages differ by one newline.
        html = re.sub(r'<!-- BEGIN crawlable nav \(generated by tools/apply_crawl_nav\.py\) -->'
                      r'.*?<!-- END crawlable nav -->\n?', '', html, flags=re.S)
        html = re.sub(r'<dc-import name="(SiteFooter(?:\.es)?)"([^>]*)>\s*</dc-import>',
                      r'<dc-import name="\1"\2></dc-import>', html)
