...
def photo_key(url):
    """A name for "the same picture", ignoring which size WordPress served.

    Retreat Guru's `gallery_images` always opens with the program's featured
    photo — the one already filling the hero — so without this every page
    repeated its own hero halfway down, directly above the register panel. It
    is not always the identical URL either: the gallery often carries a resized
    copy (`stone-tower-square-1024x1024.jpg` against the hero's
    `stone-tower-square.jpg`), so matching on the URL alone catches only some
    of them. Strip the -WxH suffix and compare filenames."""
    name = (url or '').split('/')[-1].split('?')[0]
    return re.sub(r'-\d+x\d+(?=\.[A-Za-z0-9]+$)', '', name).lower()
...
def render(p, others, fam=FAMILIES['retreat']):
    ...
    gallery = ''
    # The hero, again, is not a gallery. Retreat Guru leads `gallery_images`
    # with the featured photo, so an unfiltered strip showed every page its own
    # hero a second time, right above the register panel. On most programs that
    # duplicate is the *only* gallery image, and the section drops out
    # altogether — which is the intended outcome, not an empty strip.
    hero_key = photo_key(p.get('photo')
                         or ((p.get('photo_details') or {}).get('large') or {}).get('url'))
    shots = [g for g in (p.get('gallery_images') or []) if g
             and photo_key(g) != hero_key][:6]
    if shots:
        imgs = '\n'.join(
    ...
    # `@id` points at the Organization node apply_schema.py writes into the same
    # page's <head>. Without it the two blocks describe two organisations that
    # happen to share a name, and the whole point of the entity graph — one
    # Paititi, with a founder and a reserve and a nonprofit status, running this
    # program — is lost at exactly the page where somebody is deciding whether
    # to book. Naming it as well as pointing at it keeps this block readable on
    # its own if the head block is ever removed.
    org = {'@id': SITE + '/#organization', '@type': 'Organization',
           'name': 'Paititi Institute', 'url': SITE}
...
# apply_analytics.py adds the Google tag, the consent defaults and conversions.js
# to every page's <head> *after* this generator has run — see the pipeline order
# in CLAUDE.md. This template does not carry that block and should not: it is
# owned by one tool, in one place. But it means a freshly generated page and the
# page on disk differ by exactly that block, which made --check report all six
# retreat pages as drifting forever. A check that always fails is a check nobody
# reads, so compare with the block taken off both sides.
#
# apply_schema.py and apply_crawl_nav.py add two more — the schema.org @graph
# and the no-JavaScript navigation — for exactly the same reason and with
# exactly the same consequence, so they are stripped the same way. Note the
# @graph is *not* the Event/Course JSON-LD below, which this generator writes
# itself and which must still be compared.
DOWNSTREAM_BLOCK_RES = (
    re.compile(r'<!-- BEGIN google tag \(generated by tools/apply_analytics\.py\) -->'
               r'.*?<!-- END google tag -->\n?', re.S),
    re.compile(r'<!-- BEGIN structured data \(generated by tools/apply_schema\.py\) -->'
               r'.*?<!-- END structured data -->\n?', re.S),
    re.compile(r'<!-- BEGIN crawlable nav \(generated by tools/apply_crawl_nav\.py\) -->'
               r'.*?<!-- END crawlable nav -->\n?', re.S),
)

# The crawlable nav lives *inside* the SiteFooter import rather than between two
# elements, so taking the block out leaves the whitespace it was indented with
# behind: `<dc-import name="SiteFooter">\n\n</dc-import>` against the template's
# `<dc-import name="SiteFooter"></dc-import>`. Two pages that differ by a
# newline inside an empty element are the same page, and reporting them as drift
# would put us straight back to a check nobody reads.
EMPTY_FOOTER_RE = re.compile(
    r'<dc-import name="(SiteFooter(?:\.es)?)"([^>]*)>\s*</dc-import>')


def without_downstream(text):
    """The page as this generator would have written it, blocks owned by the
    later pipeline stages taken off."""
    for pat in DOWNSTREAM_BLOCK_RES:
        text = pat.sub('', text)
    return EMPTY_FOOTER_RE.sub(r'<dc-import name="\1"\2></dc-import>', text)
...
def write(path, content, check, changed):
    ...
    if old == content:
        return
    if old is not None and without_downstream(old) == without_downstream(content):
        # Identical but for the blocks apply_analytics and apply_schema own.
        # Nothing to report; rewriting would only strip them and make the next
        # tools put them back.
        return
